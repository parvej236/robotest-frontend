      ...
      <div class="glass-card neon-border-red p-8 relative">
        ...
        <form @submit.prevent="handleLogin" class="space-y-5">
          <div>
            ...
            <input v-model="form.email" type="email" placeholder="your@gmail.com"
                   class="input-field" :class="error ? 'border-neon-red/80' : ''" required />
          </div>
          <div>
            <label class="label-text">Password</label>
            <div class="relative">
              <input v-model="form.password" :type="showPass ? 'text' : 'password'"
                     placeholder="Type your password" class="input-field pr-10" required />
          ...
          <!-- Error -->
          <div v-if="error" class="flex items-center gap-2 px-3 py-2.5 bg-neon-red/10 border border-neon-red/30 rounded text-neon-red text-xs font-body">
        ...
        <div class="mt-6 text-center">
          <p class="text-sm font-body text-white/40">
            No account yet?
            <router-link to="/register" class="text-neon-blue hover:text-white ml-1 transition-colors">Register here</router-link>
          </p>
        </div>
      </div>
    </div>
