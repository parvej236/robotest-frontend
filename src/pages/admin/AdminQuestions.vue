  ...
  <div class="relative min-h-screen pt-20 px-4 pb-12 bg-transparent overflow-hidden">
    <div class="relative z-10 max-w-5xl mx-auto">
      ...
      <div v-else-if="questions.length === 0"
        class="rounded-3xl border border-white/10 bg-white/5 p-16 text-center backdrop-blur-xl shadow-[0_24px_80px_rgba(0,0,0,0.18)]">
        <div class="text-5xl mb-4">📋</div>
        <p class="text-slate-300 font-medium mb-4">No questions added yet for this contest.</p>
        <button @click="openAdd"
          class="inline-flex items-center justify-center rounded-2xl bg-sky-500 px-5 py-3 text-sm font-semibold text-white shadow-lg shadow-sky-500/20 transition hover:bg-sky-400">
          Add First Question
        </button>
      </div>
      ...
      <div v-else class="space-y-5">
        <div v-for="(q, idx) in questions" :key="q.id"
          ...
          <div class="px-5 py-4 border-b border-white/10 bg-slate-950/70">
            <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              <div class="flex items-center gap-4 min-w-0">
                <div
                  class="flex h-12 w-12 items-center justify-center rounded-2xl bg-sky-500/10 text-sky-200 border border-sky-500/20 font-display font-black text-base">
                  {{ idx + 1 }}
                </div>
                <div class="min-w-0">
          ...
          <div class="p-5 grid gap-4 lg:grid-cols-[360px_1fr] items-start">
            <div class="space-y-4">
              <div v-if="q.imageUrl"
                class="group relative overflow-hidden rounded-3xl border border-white/10 bg-slate-900/80 shadow-inner transition duration-300 hover:-translate-y-0.5">
                <a :href="toFullUrl(q.imageUrl)" :download="downloadFileName(q.imageUrl)" target="_blank" rel="noopener"
                ...
                <button type="button" @click="openFullImage(q.imageUrl)" class="block w-full overflow-hidden text-left">
                  <img v-if="isLikelyImageUrl(q.imageUrl)" :src="toFullUrl(q.imageUrl)" alt="Question file"
              ...
              <div v-else
                class="flex min-h-[16rem] items-center justify-center rounded-3xl border border-dashed border-white/10 bg-slate-900/80 text-sm text-slate-500">
                No file uploaded
              </div>
        ...
        <form @submit.prevent="handleSave" class="space-y-5">
          ...
          <div>
            ...
            <div class="flex items-start gap-4">
              <div class="flex-1">
                <label
                  class="flex items-center gap-3 rounded-3xl border border-white/10 bg-white/5 p-4 cursor-pointer transition hover:border-sky-400/30">
                  <svg class="w-8 h-8 text-slate-300 flex-shrink-0" fill="none" viewBox="0 0 24 24"
                    stroke="currentColor">
              ...
              <div v-if="imagePreview || (editingId && qForm.existingImageUrl)"
                ...
                <img v-if="imagePreviewIsImage || (editingId && isLikelyImageUrl(qForm.existingImageUrl))"
                  :src="imagePreview || toFullUrl(qForm.existingImageUrl)" class="w-full h-full object-cover"
                  alt="Preview" />
                <div v-else
          ...
          <div class="grid grid-cols-3 gap-4">
            ...
            <div>
              <label class="label-text">Time Limit (Seconds)</label>
              <div class="relative">
                <input v-model.number="qForm.timeLimit" type="number" min="0" class="input-field pr-8 required"
                  placeholder="e.g. 300 for 5 min" />
                <span class="absolute right-3 top-1/2 -translate-y-1/2 text-[10px] text-white/30 font-mono">SEC</span>
              </div>
              <p class="text-sm text-white mt-1 italic">
                {{ qForm.timeLimit > 0 ? formatTimeLabel(qForm.timeLimit) : 'No time limit set' }}
              </p>
            </div>
          </div>
          ...
          <div v-if="qForm.correctAnswer > 0" class="glass-card p-4 border border-neon-blue/20 bg-neon-blue/5">
            ...
            <div class="flex items-center gap-3">
              ...
              <div class="flex-1 h-1 bg-dark-800 rounded-full relative">
                <div
                  class="absolute inset-y-0 left-0 right-0 bg-gradient-to-r from-neon-red/40 via-neon-blue to-neon-red/40 rounded-full">
                </div>
                <div
                  class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-3 h-3 bg-white rounded-full border-2 border-neon-blue shadow">
                </div>
              </div>
              <div class="text-center">
          ...
          <div class="grid sm:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input v-model.number="qForm.points" type="number" min="1" class="input-field" placeholder="e.g. 10"
                required />
            </div>
          </div>
    ...
    <div v-if="showImageModal"
      ...
      <div
        class="relative w-full max-w-5xl overflow-hidden rounded-[2rem] bg-slate-950/95 p-4 shadow-[0_36px_120px_rgba(0,0,0,0.35)]">
        ...
        <img v-if="isLikelyImageUrl(imageModalUrl)" :src="toFullUrl(imageModalUrl)" alt="Full question file"
          class="mx-auto max-h-[75vh] w-full object-contain" />
        <iframe v-else-if="isLikelyPdfUrl(imageModalUrl)" :src="toFullUrl(imageModalUrl)"
          class="mx-auto w-full max-h-[75vh] rounded-lg border border-white/10 bg-white/5" />
        <div v-else class="mx-auto w-full max-w-2xl text-center text-white/70 py-12">
...
async function handleSave() {
  ...
  try {
    ...
    fd.append('question', new Blob([JSON.stringify({
      ...
      type: 'NUMERIC_MCQ',
      timeLimit: qForm.value.timeLimit || 0,
...
const formatTimeLabel = (seconds) => {
  ...
  const s = seconds % 60;
  return `Equivalent to: ${h > 0 ? h + 'h ' : ''}${m > 0 ? m + 'm ' : ''}${s > 0 ? s + 's' : ''}`;
}
</script>