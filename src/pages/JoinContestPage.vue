<template>
  <div class="min-h-screen bg-transparent text-slate-200 selection:bg-neon-blue/30 overflow-hidden flex flex-col">
    ...
    <nav
      class="h-20 bg-black/60 backdrop-blur-md border-b border-white/5 px-6 flex items-center justify-between shrink-0 z-50">
      <div class="flex items-center gap-6">
        <router-link :to="`/contests/${$route.params.id}`"
          class="text-xs font-display uppercase tracking-widest text-orange-500 hover:text-white transition-colors">
          ← Exit
        </router-link>
    ...
    <div v-else-if="submitted" class="flex-1 flex items-center justify-center p-6">
      <div class="glass-card neon-border-blue p-12 text-center max-w-md">
        ...
        <h2 class="font-display text-3xl font-black text-neon-blue uppercase tracking-tighter mb-4">Submission Received
        </h2>
    ...
    <div v-else class="flex-1 flex flex-col md:flex-row overflow-hidden">
      ...
      <aside
        class="w-full md:w-24 bg-white/[0.02] border-r border-white/5 overflow-y-auto p-4 flex md:flex-col gap-3 custom-scrollbar">
        <button v-for="(q, idx) in questions" :key="q.id" @click="activeIdx = idx"
          ...
          {{ idx + 1 }}
          <div v-if="isAnswered(q.id) && activeIdx !== idx"
            class="absolute -top-1 -right-1 w-3 h-3 bg-neon-blue rounded-full border-2 border-[#050505]"></div>
        </button>
      </aside>
          ...
          <Transition name="fade-slide" mode="out-in">
            <div :key="activeIdx" class="space-y-8">
              ...
              <div class="max-w-xl space-y-4">
                ...
                <p v-if="questions[activeIdx].type === 'NUMERIC_MCQ' && questions[activeIdx].errorPercentage"
                  class="text-xl font-mono text-blue-500">
                  Tolerance: ±{{ questions[activeIdx].errorPercentage }}%
                </p>
              </div>

              <div class="pt-10 flex items-center justify-between border-t border-white/5">
                <button @click="activeIdx--" :disabled="activeIdx === 0"
                  class="text-xs font-display uppercase tracking-widest text-green-500 hover:text-white disabled:opacity-0">
                  ← Previous
                </button>

                <button v-if="activeIdx < questions.length - 1" @click="activeIdx++" class="btn-primary px-10">
                  Next Question
                </button>

                <button v-else @click="handleSubmit" :disabled="submitting || answeredCount === 0"
...
onMounted(async () => {
  try {
    ...
    // const registered = await contestStore.isRegistered(contestId)
    // const alreadySubmitted = await contestStore.hasSubmitted(contestId)

    // if (!registered || alreadySubmitted) {
